...
namespace KNFE.Core.Format.Archive
{
    ...
    public class Fallout1DatFormat : FileFormat
    {
            ...
            : base("Fallout 1 DAT", fileName)
        {
            ...
            if (_dirCount < 1)
                throw new InvalidDataException($"Received invalid directory count ({_dirCount}) within a Fallout 1 DAT file.");

            _root = new Fallout1DatEntry("");
            ...
            for (int i = 0; i < _dirCount; i++)
            {
                ...
                _root.AddChild(new Fallout1DatEntry(tempName));
            }

            // Read directory contents block
            foreach (Fallout1DatEntry dir in _root._children)
            {
                // Get file count of the current directory
                int dirFileCount = BinaryPrimitives.ReverseEndianness(_br.ReadInt32());
                if (dirFileCount < 1)
                    throw new InvalidDataException($"Received invalid directory file count ({dirFileCount}) within a Fallout 1 DAT file entry.");
                ...
                for (int i = 0; i < dirFileCount; i++)
                {
                    ...
                    Fallout1DatEntry tempEntry = new Fallout1DatEntry(tempFileName, InFileStream);
                    ...
                    switch (compFlags)
                    {
                        ...
                        case FLAG_TEXT:
                            ...
                            break;
                        default:
                            throw new InvalidDataException($"Received invalid compression flag (0x{Convert.ToString(compFlags, 16).ToUpper()}) within a Fallout 1 DAT file entry.");
                    }
                    ...
                    if (tempEntry._offset > _br.BaseStream.Length)
                        throw new InvalidDataException($"Received invalid data offset (0x{Convert.ToString(tempEntry._offset, 16).ToUpper()}) within a Fallout 1 DAT file entry.");
                    ...
                    if (tempEntry._compressedLength + tempEntry._offset > _br.BaseStream.Length)
                        throw new InvalidDataException($"Received invalid data length (0x{Convert.ToString(tempEntry._compressedLength, 16).ToUpper()}) within a Fallout 1 DAT file entry.");
        ...
        {
            // New root
            Fallout1DatEntry tempRoot = new Fallout1DatEntry("");
            // For stylistic ordering, we note the pseudo-root (directory ".") to add files to bottom of tree
            Fallout1DatEntry pseudoRoot = null;

            foreach (Fallout1DatEntry fe in _root.Children)
            {
                ...
                Fallout1DatEntry curEntry = tempRoot;
                while (dirBuffer.Length != 0)
                {
                    ...
                    Fallout1DatEntry tempEntry = (Fallout1DatEntry)curEntry.GetChild(curDir);
                    // If not, make it and add to tree
                    if (tempEntry == null)
                    {
                        tempEntry = new Fallout1DatEntry(curDir);
                ...
                // Add files
                foreach (Fallout1DatEntry fde in fe.Children)
                {
            ...
            if (pseudoRoot != null)
            {
                foreach (Fallout1DatEntry fe in pseudoRoot.Children)
                {
